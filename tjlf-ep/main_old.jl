# This file was used as my old main.jl. I first just translated the driver in TGLF-EP as the main, but as that should be done elsewhere
# I have since updated the scheme. The driver is what is run from a local directory while the main.jl is done via REPL in Julia,
# so the code can be ran as a line-by-line script or as a single function.
...
# These (above) should be set from the working directory, but these test cases are good for now:
