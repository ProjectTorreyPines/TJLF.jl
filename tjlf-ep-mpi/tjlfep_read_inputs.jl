...
function TJLF_map(inputsEP::InputTJLFEP{Float64}, inputsPR::profile{Float64}, testid::Bool)
    #=
    # Temp Defs:
    color = 0
    kyhat_in = 3
    ...
    # Print before scan_method:
    if (testid)
        # This should be identical for ALL scan values of ky.
        #println("Step 1: ", inputTJLF.AS)
        # It is correct even between the two process versions.
    end

    if (inputsEP.SCAN_METHOD == 1)
        # This does happen:
        inputTJLF.AS[is] = inputsPR.AS[ir, is]*inputsEP.FACTOR_IN
        # This is incorrect. inputsEP.FACTOR_IN must be incorrect, then.
        # To make sure, I will check all of these values:
    end

    # Print after scan_method:
    if (testid)
        # This should be identical for ALL scan values of ky.
        #println("Step 2: ", inputTJLF.AS, " : ", is, " : ", inputsPR.AS[ir, is], " : ", inputsEP.FACTOR_IN)
    end
    ...
    # Print after a_qn scaling:
    if (testid)
        # This should be identical for ALL scan values of ky.
        #println("Step 3: ", inputTJLF.AS, " : ", inputsPR.A_QN)
    end
